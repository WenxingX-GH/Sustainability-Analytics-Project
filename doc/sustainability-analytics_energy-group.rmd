...
# 2. Aggregate to find daily surplus (charge potential) and deficit (discharge demand)
...
# 3. Define a scaling factor to properly display the secondary y-axis
# This ensures both the daily and cumulative values are visible on the plot
scaling_factor <- max(daily_battery_potential$cumulative_surplus_kwh, na.rm = TRUE) / 
                  max(daily_battery_potential$charge_potential_kwh, na.rm = TRUE)

# 4. Plot the results
ggplot(daily_battery_potential, aes(x = date)) +
  # Daily Surplus and Deficit as lines
  ...
  # Cumulative Annual Surplus as a filled area, scaled to fit the plot
  ...
  # Manual scales for colors and fills for a clear legend
  scale_color_manual(
    name = "Daily Metric",
    values = c(

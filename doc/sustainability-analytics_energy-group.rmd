...
```{r}
df <- data.frame(
  Dataset = c("**Household Energy Profile**", "**Historical Weather Data**"),
  Provider = c("AEW Energie AG", "MeteoSwiss"),
  Description = c(
    "High-resolution energy metrics for a residential property (Wynemattestrasse 17) over ~5 years: generation, consumption, and grid interaction.",
    "Hourly weather observations (OGD) used to correlate energy patterns with environmental conditions."
  ),
  `Key variables used` = c(
    "`pv_production_k_wh` – PV energy production (kWh)<br>`consumption_base_load_k_wh` – Base load (e.g., refrigerator, standby)<br>`consumption_heat_pump_k_wh` – Heat pump consumption<br>`consumption_ev_charging_k_wh` – EV charging consumption<br>`grid_feed_in_pv_k_wh` – Surplus PV fed into grid<br>`grid_import_total_k_wh` – Total grid import",
    "`temperature_c` – Mean air temperature (°C)<br>`humidity_pct` – Mean relative humidity (%)<br>`wind_speed_ms` – Mean wind speed (m/s)"
  ),
...
plot_vars <- c(
  "consumption_base_load_k_wh", "consumption_heat_pump_k_wh", "consumption_cooking_lighting_etc_k_wh",
...
monthly_summary <- final_df_d %>%
  ...
  summarise(
    ...
    consumption_cooking_lighting_kwh = sum(consumption_cooking_lighting_etc_k_wh, na.rm = TRUE),
...
monthly_long <- monthly_summary %>%
  ...
  mutate(
    category = recode(category,
      ...
      consumption_cooking_lighting_kwh = "Cooking & lighting"
    ),
...
### 4.4 Pairs Plot Weather and Energy Variables
To gain a first impression of how the weather variables influence our energy variables, we created a pairs plot that visualizes the relationships between each variable. More discuss about the pairs plot see in `6 Modeling`.
...
df3 <- final_df_h %>%
  ...
  transmute(
    ...
    cooking = consumption_cooking_lighting_etc_k_wh,
    ev   = consumption_ev_charging_k_wh,
    pv   = pv_production_k_wh
  ) %>%
  mutate(across(c(base, heat, cooking, ev, pv), ~replace_na(., 0)),
         total_cons = base + heat + cooking + ev,
         net = pv - total_cons)

# Consumption components (as negatives for stacking)
cons_long <- df3 %>%
  select(timestamp, year, base, heat, cooking, ev) %>%
...
p_balance <- ggplot() +
  ...
  guides(
    ...
    fill     = guide_legend(nrow = 2, byrow = TRUE, override.aes = list(size = 6)),
...
The comparison of April 2022 (before EV adoption) and April 2024 (after EV introduction) highlights the impact of EV charging on household energy balance. In April 2024, EV charging accounted for roughly 18% of total household electricity consumption, representing a significant new load on the system. Importantly, most charging sessions occurred between 9 pm and 3 am, a period without PV production, which led to a strongly negative net balance and increased reliance on grid imports. This highlights a clear optimization opportunity: shifting charging into daylight hours could allow EV demand to be met by self-generated solar energy, improving self-consumption and reducing grid dependency.

The table `EV Charging Share of Total Household Consumption (%)` summarizes the monthly share of household electricity consumption attributed to EV charging in 2023 and 2024. In both years, EV charging demand is relatively low during the winter months (8–12%), but it rises sharply in summer, peaking at 37.0% in August 2023 and 35.6% in August 2024. This strong seasonal pattern indicates that EV charging makes up over one-third of total household electricity use in summer, while in winter it remains closer to 10% of the load. 


```{r, EV Charging Share of Total}

# ---- Prepare monthly EV share ----
ev_monthly_pct <- final_df_h %>%
  filter(year(timestamp) %in% c(2023, 2024)) %>%
  mutate(year  = year(timestamp),
         month = month(timestamp, label = TRUE, abbr = TRUE)) %>%
  group_by(year, month) %>%
  summarise(
    ev_total   = sum(consumption_ev_charging_k_wh,           na.rm = TRUE),
    base_total = sum(consumption_base_load_k_wh,             na.rm = TRUE),
    heat_total = sum(consumption_heat_pump_k_wh,             na.rm = TRUE),
    cook_total = sum(consumption_cooking_lighting_etc_k_wh,  na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    total_cons = ev_total + base_total + heat_total + cook_total,
    ev_pct     = round(100 * ev_total / total_cons, 2)
  ) %>%
  select(year, month, ev_pct) %>%
  arrange(year, month)

# ---- Render as table ----
ev_monthly_pct %>%
  tidyr::pivot_wider(names_from = month, values_from = ev_pct) %>%
  kable(
    caption = "EV Charging Share of Total Household Consumption (%)",
...
The chart `Annual PV Production Breakdown with Grid Import Overlay` compares PV production (split into self-consumption and grid feed-in) with total grid imports from 2020 to 2024. PV production remains relatively stable at around 5,800–6,000 kWh annually, with no significant increase in self-consumption across the years.

In contrast, grid imports rise sharply by about 2,500 kWh between 2022 and 2023, a trend that coincides with the introduction of EV charging. This indicates that the additional electricity demand from EV charging is being supplied almost entirely by grid imports rather than local PV production, since PV self-consumption remains unchanged.

This highlights a critical inefficiency: EV charging demand is not aligned with PV generation, leaving the household more dependent on external electricity sources despite stable solar output.

```{r, Annual PV Production Breakdown with Grid Import Overlay}

#  Aggregate hourly -> annual
annual <- final_df_h %>%
  ...
  mutate(check_diff_kwh = pv_production_k_wh -
           (self_consumption_pv_k_wh + grid_feed_in_pv_k_wh))
...
ggplot() +
  ...
  geom_line(
    data = annual,
    aes(x = factor(year), y = grid_import_total_k_wh,
        group = 1, color = "Grid import (total)"),
    linewidth = 1
  ) +
  geom_point(
    data = annual,
    aes(x = factor(year), y = grid_import_total_k_wh,
        color = "Grid import (total)"),
    size = 2.2
  ) +
  scale_y_continuous(labels = scales::label_number(
    scale_cut = scales::cut_si("k"))) +
...
-   Shift Charging to Daytime Hours: Most EV charging currently occurs at night (9 pm–3 am), when PV production is zero. Rescheduling charging to mid-day (10 am–4 pm), especially in spring and summer, would allow a significant portion of the EV demand to be met directly from PV generation @Fachrizal2020.

-   Smart Charging / Automated Scheduling: Use smart chargers or time-based charging programs to automatically prioritize charging during periods of surplus solar output @Ye2024. This reduces reliance on grid imports and minimizes energy wasted in feed-in (low compensation tariffs).

-   Flexible Charging Strategies: Instead of fully charging the EV overnight, apply partial daytime charging when PV is available. Example: If the EV is parked at home during work-from-home days or weekends, charge incrementally during peak PV hours.

-   Battery Storage Integration: If daily charging flexibility is limited (e.g., car is absent during daytime), adding a home battery could store excess solar from mid-day and discharge it during evening EV charging. This increases PV self-consumption not only for the EV but also for other household loads @Gu2025, @Ullner2025.

-   Dynamic Tariff and Grid Optimization: Combine smart charging with real-time electricity price signals, Use cheap off-peak grid electricity at night only when PV availability is insufficient. Prioritize solar charging whenever possible to reduce long-term electricity costs.
...
```{r}
# 1. Define all possible consumption columns
# This list now correctly includes the "cooking & lighting" category.
consumption_cols <- c(
  "consumption_base_load_k_wh",
  "consumption_heat_pump_k_wh",
  "consumption_ev_charging_k_wh",
...
# 2. Find which of these columns actually exist in your dataframe
existing_consumption_cols <- intersect(consumption_cols, names(final_df_h))

# 3. Stop with a clear error message if no consumption data is found
if (length(existing_consumption_cols) == 0) {
  stop("CRITICAL ERROR: No consumption columns found in the data. Please check the input file.")
}

# 4. Calculate hourly net energy using a robust method
hourly_net <- final_df_h %>%
  # First, ensure all numeric columns have no NA values
  mutate(across(where(is.numeric), ~replace_na(., 0))) %>%
  # Now, calculate total consumption by summing ONLY the columns that exist.
  # This use of rowSums() is the key to making the code robust.
  mutate(
    total_consumption = rowSums(select(., all_of(existing_consumption_cols))),
    net_kwh = pv_production_k_wh - total_consumption
  )

# 5. Aggregate to find daily surplus (charge potential) and deficit (discharge demand)
daily_battery_potential <- hourly_net %>%
  group_by(date) %>%
  summarise(
    charge_potential_kwh = sum(net_kwh[net_kwh > 0], na.rm = TRUE),
    discharge_demand_kwh = abs(sum(net_kwh[net_kwh < 0], na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  arrange(date) %>%
  group_by(year = year(date)) %>%
  mutate(cumulative_surplus_kwh = cumsum(charge_potential_kwh)) %>%
  ungroup()

# 6. Define the scaling factor for the secondary y-axis (with safety check)
max_charge_potential <- max(daily_battery_potential$charge_potential_kwh, na.rm = TRUE)
scaling_factor <- if (is.finite(max_charge_potential) && max_charge_potential > 0) {
  max(daily_battery_potential$cumulative_surplus_kwh, na.rm = TRUE) / max_charge_potential
} else {
  1 # Default to 1 if no valid charge potential exists to prevent errors
}
...
ggplot(daily_battery_potential, aes(x = date)) +
  ...
  geom_area(aes(y = cumulative_surplus_kwh / scaling_factor, fill = "Cumulative Annual Surplus"), alpha = 0.3) +
  scale_y_continuous(
  ...
  scale_color_manual(
    name = "Daily Metric",
    values = c("Daily Surplus (Charge Potential)" = "darkgreen", "Daily Deficit (Discharge Demand)" = "darkred")
  ) +
  scale_fill_manual(
    name = "Annual Metric",
    values = c("Cumulative Annual Surplus" = "steelblue")
  ) +
  labs(
...
Although more advanced models could potentially achieve higher accuracy, many of the variables required for such approaches (e.g., solar irradiance) were not available in our dataset. Therefore, linear regression was chosen as a suitable compromise: simple, interpretable, and appropriate given the limited input variables.
...
#Output table
modelsummary(
  list("Heat Pump Consumption" = model_hp,
       "PV Production" = model_pv),
  stars = TRUE,
  statistic = "std.error",
  gof_omit = "IC|Log|F",
  output = "gt",  
  coef_map = c("(Intercept)" = "Intercept",
               "temperature_daily_C" = "Temperature (°C)")
)
...
It consists of four main views: a daily perspective, an hourly perspective, a forecast view, and a page with analytics. 

We decided to include only a few key variables to keep it clear and easy to understand. We chose Heat Pump Consumption because, compared to the other variables, it accounts for the highest energy use — almost as if we were showing the entire energy consumption of the house. In addition, we included PV Production and Electric Vehicle consumption, since they represent major and highly variable factors in the household’s overall energy balance.

Together, these pages illustrate how temperature affects both PV production and heat pump consumption, and how future values can be estimated based on weather forecasts. In the following sections, each dashboard view is presented, along with an explanation of the steps taken to create them in Tableau.
...
In this project, we used AI tools (mainly ChatGPT) to support code revision and report writing.
Examples include:

-   Refining code syntax and improving ggplot visualizations

-   Debugging errors and warnings

-   Clarifying explanations, such as model interpretations

All AI suggestions were tested and reviewed before inclusion to ensure accuracy. Overall, AI was helpful, but careful verification was essential.
